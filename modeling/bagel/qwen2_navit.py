...
def flash_attn_varlen_func(
    ...
    if q.dim() != 3 or k.dim() != 3 or v.dim() != 3:
        raise ValueError("Expected q,k,v to be [T, H, D] tensors.")

    if k.shape[1] != q.shape[1]:
        if q.shape[1] % k.shape[1] != 0:
            raise ValueError("Unsupported GQA layout for fallback attention.")
        rep = q.shape[1] // k.shape[1]
        k = k.repeat_interleave(rep, dim=1)
        v = v.repeat_interleave(rep, dim=1)

    cuq = cu_seqlens_q.to(torch.long)
    cuk = cu_seqlens_k.to(torch.long)
    bsz = int(cuq.numel() - 1)
    outs: List[torch.Tensor] = []
    for i in range(bsz):
        qs, qe = int(cuq[i].item()), int(cuq[i + 1].item())
        ks, ke = int(cuk[i].item()), int(cuk[i + 1].item())
        q_i = q[qs:qe].transpose(0, 1).unsqueeze(0)
        k_i = k[ks:ke].transpose(0, 1).unsqueeze(0)
        v_i = v[ks:ke].transpose(0, 1).unsqueeze(0)
        out_i = scaled_dot_product_attention(q_i, k_i, v_i, attn_mask=None, dropout_p=0.0, is_causal=causal)
...
class PackedAttention(Qwen2Attention):
    ...
    def forward_inference(
        ...
        recorder = getattr(self, "attention_recorder", None)
        if recorder is not None and recorder.enabled:
            key_states_for_attn = merged_key_states
            if self.num_key_value_heads != self.num_heads:
                key_states_for_attn = key_states_for_attn[:, :, None, :].repeat(1, 1, self.num_key_value_groups, 1)
                key_states_for_attn = key_states_for_attn.reshape(-1, self.num_heads, self.head_dim)
            query_states_for_attn = packed_query_states
            if query_states_for_attn.dtype != torch.float32:
                query_states_for_attn = query_states_for_attn.float()
            if key_states_for_attn.dtype != torch.float32:
                key_states_for_attn = key_states_for_attn.float()
            attn_scores = torch.einsum("qhd,khd->hqk", query_states_for_attn, key_states_for_attn)
            if is_causal:
                key_positions = torch.arange(key_states_for_attn.shape[0], device=attn_scores.device)
                query_positions = packed_query_indexes.to(attn_scores.device)
                causal_mask = key_positions.unsqueeze(0) <= query_positions.unsqueeze(1)
                attn_scores = attn_scores.masked_fill(~causal_mask.unsqueeze(0), float("-inf"))
            attn_probs = torch.softmax(attn_scores, dim=-1)
            recorder.record(self.layer_idx, attn_probs, packed_query_indexes, key_states_for_attn.shape[0])
