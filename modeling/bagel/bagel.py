...
class Bagel(PreTrainedModel):
    ...
    def forward(
        ...
        return dict(mse=mse, ce=ce, logits=logits)
    ...
    def forward_cache_update_vit(
        ...
        cu_seqlens = cu_seqlens.to(torch.int32)
        max_seqlen = torch.max(vit_token_seqlens).item()
        vit_embed_weight = self.vit_model.vision_model.embeddings.patch_embedding.weight
        if packed_vit_tokens.dtype != vit_embed_weight.dtype or packed_vit_tokens.device != vit_embed_weight.device:
            packed_vit_tokens = packed_vit_tokens.to(
                device=vit_embed_weight.device,
                dtype=vit_embed_weight.dtype,
            )
        packed_vit_token_embed = self.vit_model(
            packed_pixel_values=packed_vit_tokens, 
            packed_flattened_position_ids=packed_vit_position_ids,
    ...
    def forward_cache_update_vae(
        ...
        if packed_latent.dtype != target_dtype:
            packed_latent = packed_latent.to(target_dtype)
        if packed_pos_embed.dtype != target_dtype:
            packed_pos_embed = packed_pos_embed.to(target_dtype)
        if packed_timestep_embeds.dtype != target_dtype:
            packed_timestep_embeds = packed_timestep_embeds.to(target_dtype)
    ...
    def next_token_logits(
        ...
        embed_weight_device = self.language_model.model.embed_tokens.weight.device
        if embed_weight_device.type != "meta":
            if packed_start_tokens.device != embed_weight_device:
                packed_start_tokens = packed_start_tokens.to(embed_weight_device)
            if packed_query_position_ids.device != embed_weight_device:
                packed_query_position_ids = packed_query_position_ids.to(embed_weight_device)
            if key_values_lens.device != embed_weight_device:
                key_values_lens = key_values_lens.to(embed_weight_device)
            if packed_key_value_indexes.device != embed_weight_device:
                packed_key_value_indexes = packed_key_value_indexes.to(embed_weight_device)

        packed_text_embedding = self.language_model.model.embed_tokens(packed_start_tokens)
