#!/bin/bash
# Visual-OPSD on-policy training launcher (paper default).
#
# For each training step and each raw sample:
#   1. Student samples a full completion (reasoning + answer) from its
#      current policy via FSDP.summon_full_params + BAGEL inference
#      primitives.
#   2. Same completion is packed into student / teacher batches.  The
#      teacher's privileged channel is STRICTLY visual-only: the VT
#      images appear, but the text thoughts and ground-truth answer do
#      not.  This isolates the visual generation pathway as the source
#      of the distillable distributional gap.
#   3. Dual forward, JSD over the ENTIRE completion span (pure JSD by
#      default: ce_weight=0, jsd_weight=1).
#   4. Backward + EMA update (decay 0.995).
...
# TEACHER_MODE=${2:-fixed}                # self | ema | fixed
TEACHER_MODE=${2:-ema}                # self | ema | fixed
JSD_BETA=${3:-0.5}
JSD_TEMP=${4:-1.0}
JSD_WEIGHT=${5:-1.0}
TOTAL_STEPS=${6:-2000}
OUTPUT_NAME=${7:-visual-opsd-${TEACHER_MODE}-beta${JSD_BETA}}
...
# Number of <image_end> injections allowed to skip the image-generation
# stage when the overfit policy stops at <image_start>.  See
# scripts/visual_opsd/on_policy_sampler.py (_generate_single) for details.
...
echo "=== Visual-OPSD On-Policy Training ==="
echo "Model: ${MODEL_PATH}"
echo "Teacher: ${TEACHER_MODE} (ema_decay=${EMA_DECAY})"
